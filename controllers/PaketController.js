...
exports.tambahpaket = function(req, res){
    ...
    connection.query('INSERT INTO paket(nama_paket,harga_paket,jumlah_data,jumlah_telpon,masa_aktif) VALUES(?,?,?,?,?)',
...
exports.ubahpaket = function(req, res){
    ...
    connection.query('UPDATE paket SET nama_paket=?, harga_paket=?, jumlah_data=?, jumlah_telpon=?, masa_aktif=? WHERE id_paket=?', [namapaket, hargapaket, jumlahdata, jumlahtelpon, masaaktif, idpaket],
        function(error, rows, fields){
            if(error){
                console.log(error);
                res.status(500).json({ message: 'Gagal mengubah data paket', error: error.message });
            } else {
...
exports.hapuspaket = function(req, res){
    var idpaket = req.params.idpaket;
    connection.query('DELETE FROM paket WHERE id_paket=?', [idpaket],
        function(error, rows, fields){
            if(error){  
                console.log(error);
                res.status(500).json({ message: 'Gagal menghapus data paket', error: error.message });
            } else {
